## ChemSmart Studio operating contract

This repository is the AGPL-3.0 ChemSmart Studio product pinned by
`upstreams.lock.json`. The inherited provider, window, lifecycle, data, and UI
foundations remain internal implementation. **Electron main owns the molecule
document and the renderer draws it with Three.js**, ChemSmart owns the Agent loop
and safety gates, and `schemas/v2/` is the source of truth for active
process-boundary contracts. v1 decoders are historical read compatibility only.

> **The native floor is retired.** `MoleculeDocumentService` owns molecule state,
> `packages/chem-molecular-engine` draws it with Three.js, and the Python sidecar
> owns trajectory storage and molecular import. Do not reintroduce Qt, Avogadro,
> IOSurface, UFF, or fork-owned native C++.

Before changing code, use the canonical `$chemsmart-studio` skill, inspect the
branch and dirty status, submodule hash, `upstreams.lock.json`, and the nearest
`AGENTS.md`. Preserve unrelated user work.

Never:

- edit `vendor/` directly or copy code from a dirty ChemSmart checkout;
- read or print `api.env`, tokens, socket credentials, or model provider secrets;
- run a calculation without exact one-shot approval, or publish a draft without
  the appropriate Save/Run review boundary;
- expose Node APIs directly to the renderer or bypass typed `IpcApi` routes;
- hand-edit generated TypeScript/Python protocol models.

Every main-process IPC input must be schema validated. User-facing strings use
i18n and logs use the central logger. Formatting and code generation must first
be scoped to the intended files. Each milestone records schema, unit,
integration, and package gate results (including failures) as a JSON receipt.

Local rules are intentionally narrow: `services/AGENTS.md` covers
AgentSession/gates, and `schemas/AGENTS.md` covers schema
compatibility/versioning.

### Human surfaces vs agent surfaces

Studio has four panes, and the distinction between them decides what may carry a
filesystem path and what may run without approval.

| Pane | Surface | Component |
|---|---|---|
| Left — project explorer | human | `ResearchRail.tsx` |
| Middle — 3D stage, tabs, build tools | human | `MoleculeStage.tsx`, `MoleculeTabs.tsx`, `MoleculeBuildTools.tsx` |
| Bottom — command console | human | `CommandConsole.tsx` |
| Right — ChemSmart Agent | agent | `ChemSmartAgentPane.tsx` |

- **Paths.** Human surfaces may show real paths; the explorer's roots come from the
  one deliberate egress route, `chemsmart_studio.workspace.roots`. Agent-facing
  contracts stay path-free — `command-synthesis.schema.json` forbids path separators
  and `project_workspace.py` exposes names only. A project is named to the renderer
  by the opaque handle `projectFiles.projectHandleId` (a digest of the path); main
  holds the only reverse map and refuses a handle it never issued.
- **Approval.** A command the researcher types in the console runs with no approval —
  it is their machine. The agent cannot reach the console: agent execution goes
  through the approval-gated tool path only, and `StudioConsoleService` is not
  exposed to the tool loop.
- **Agent capability and approval.** Main narrows each turn to `inspect`, `plan`,
  or `act`. Researcher and Agent molecule changes accumulate in the recoverable
  draft without per-edit approval; Save/Run review publishes one durable revision.
  Calculations, HPC submission, and generic command execution always require an
  exact one-shot approval. An unclassified tool is unavailable by omission.
