...
const retiredConsumerCommands = new Set<CommandId>([
  'quick_assistant.toggle',
  'selection.capture_text',
  'selection.toggle'
])

const mainKeybindings = REGISTERED_KEYBINDINGS.filter(
  (rule) => rule.scope !== 'renderer' && !retiredConsumerCommands.has(rule.command)
)
