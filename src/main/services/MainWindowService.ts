...
import { type ManagedWindow, WindowType } from '@main/core/window/types'
...
const RENDERER_RECOVERY_TIMEOUT_MS = 5_000
...
interface RendererRecoveryWindowState {
  focused: boolean
  fullscreen: boolean
  maximized: boolean
  minimized: boolean
...
export class MainWindowService extends BaseService {
  ...
  private lastHandledCrashedWebContentsId: number | null = null
  private rendererCrashRecovery: Promise<void> | null = null
  ...
  protected async onInit() {
    ...
    this.registerDisposable(
      windowManager.onWindowDestroyedByType(WindowType.Main, ({ window }) => {
  ...
  private setupMainWindowMonitor(mainWindow: BrowserWindow) {
    let lastLiveRendererIdentity = this.captureRendererIdentity(mainWindow)
    mainWindow.webContents.on('did-finish-load', () => {
      lastLiveRendererIdentity = this.captureRendererIdentity(mainWindow) ?? lastLiveRendererIdentity
    })
    mainWindow.webContents.on('render-process-gone', (_, details) => {
      logger.error(`Renderer process crashed with: ${JSON.stringify(details)}`)
      this.handleRendererProcessGone(mainWindow, lastLiveRendererIdentity)
    })
  }

  private handleRendererProcessGone(
    crashedWindow: BrowserWindow,
    lastLiveRendererIdentity: RendererIdentity | null
  ): void {
    const crashedWebContentsId = crashedWindow.webContents.id
    if (this.lastHandledCrashedWebContentsId === crashedWebContentsId) {
      return
    }
    this.lastHandledCrashedWebContentsId = crashedWebContentsId

    const currentTime = Date.now()
    const lastCrashTime = this.lastRendererProcessCrashTime
    this.lastRendererProcessCrashTime = currentTime
    if (lastCrashTime !== 0 && currentTime - lastCrashTime <= 60 * 1000) {
      logger.error('A fresh main renderer crashed during the bounded recovery window')
      application.forceExit(1)
      return
    }

    if (this.rendererCrashRecovery) {
      logger.error('A distinct main renderer crashed while recovery was already active')
      application.forceExit(1)
      return
    }

    const recovery = this.replaceCrashedMainRenderer(crashedWindow, lastLiveRendererIdentity)
    this.rendererCrashRecovery = recovery
    void recovery
      .catch((error) => {
        logger.error('Failed to replace the crashed main renderer', error as Error)
        application.forceExit(1)
      })
      .finally(() => {
        if (this.rendererCrashRecovery === recovery) {
          this.rendererCrashRecovery = null
        }
      })
  }

  private async replaceCrashedMainRenderer(
    crashedWindow: BrowserWindow,
    lastLiveRendererIdentity: RendererIdentity | null
  ): Promise<void> {
    const windowManager = application.get('WindowManager')
    const crashedWindowId = windowManager.getWindowId(crashedWindow)
    if (!crashedWindowId) {
      throw new Error('The crashed main window is not managed')
    }
    const windowState = this.snapshotWindowState(crashedWindow)

    const oldWebContents = crashedWindow.webContents
    const destroyedWindow = this.waitForManagedMainWindow(
      (managed) => managed.id === crashedWindowId && managed.window === crashedWindow,
      'crashed main window destruction',
      'destroyed'
    )
    if (!windowManager.close(crashedWindowId)) {
      throw new Error('WindowManager refused to close the crashed main window')
    }
    await destroyedWindow
    await this.destroySurvivingWebContents(oldWebContents)

    const freshManagedWindow = this.waitForManagedMainWindow(
      (managed) => managed.id !== crashedWindowId && managed.window.webContents.id !== oldWebContents.id,
      'fresh main window creation',
      'created',
      (managed) => this.restoreWindowStateAfterRendererReplacement(managed.window, windowState)
    )
    this.openMainWindow()
    const { window: freshWindow } = await freshManagedWindow
    const freshRendererIdentity = await this.waitForFreshRendererIdentity(freshWindow)
    if (
      freshWindow.webContents.id === oldWebContents.id ||
      (lastLiveRendererIdentity && this.sameRendererIdentity(freshRendererIdentity, lastLiveRendererIdentity))
    ) {
      throw new Error('Renderer recovery did not create a fresh renderer identity')
    }

    logger.info('Fresh main renderer created after renderer crash')
  }

  private captureRendererIdentity(window: BrowserWindow): RendererIdentity | null {
    ...
    return { webContentsId, rendererProcessId }
  }

  private sameRendererIdentity(left: RendererIdentity, right: RendererIdentity): boolean {
    return left.webContentsId === right.webContentsId && left.rendererProcessId === right.rendererProcessId
  }

  private snapshotWindowState(window: BrowserWindow): RendererRecoveryWindowState {
    return {
  ...
  private restoreWindowStateAfterRendererReplacement(window: BrowserWindow, state: RendererRecoveryWindowState): void {
  ...
  private async destroySurvivingWebContents(webContents: WebContents): Promise<void> {
    if (webContents.isDestroyed()) return
    const destroyed = this.waitForWebContentsEvent(webContents, 'destroyed', 'crashed webContents destruction')
    webContents.close({ waitForBeforeUnload: false })
    await destroyed
  }

  private async waitForFreshRendererIdentity(window: BrowserWindow): Promise<RendererIdentity> {
    const immediate = this.captureRendererIdentity(window)
    if (immediate) return immediate

    await this.waitForWebContentsEvent(window.webContents, 'did-finish-load', 'fresh renderer readiness')
    const identity = this.captureRendererIdentity(window)
    if (!identity) {
      throw new Error('The fresh main window has no live renderer identity')
    }
    return identity
  }

  private waitForWebContentsEvent(
    webContents: WebContents,
    event: 'destroyed' | 'did-finish-load',
    label: string
  ): Promise<void> {
    ...
    return this.withRendererRecoveryTimeout(operation, label).finally(() => {
      if (!listener) return
      if (event === 'destroyed') {
  ...
  private waitForManagedMainWindow(
    predicate: (managed: ManagedWindow) => boolean,
    label: string,
    event: 'created' | 'destroyed',
    onMatch?: (managed: ManagedWindow) => void
  ): Promise<ManagedWindow> {
    const windowManager = application.get('WindowManager')
    let subscription: { dispose(): void } | null = null
    const operation = new Promise<ManagedWindow>((resolve) => {
      const listener = (managed: ManagedWindow) => {
        if (predicate(managed)) {
          onMatch?.(managed)
          resolve(managed)
        }
      }
      subscription =
        event === 'created'
          ? windowManager.onWindowCreatedByType(WindowType.Main, listener)
          : windowManager.onWindowDestroyedByType(WindowType.Main, listener)
    })
    return this.withRendererRecoveryTimeout(operation, label).finally(() => subscription?.dispose())
  }

  private async withRendererRecoveryTimeout<T>(operation: Promise<T>, label: string): Promise<T> {
    let timer: NodeJS.Timeout | null = null
    const timeout = new Promise<never>((_, reject) => {
      timer = setTimeout(() => reject(new Error(`${label} timed out`)), RENDERER_RECOVERY_TIMEOUT_MS)
      timer.unref()
    })
    try {
