...
describe('navigate protocol handler', () => {
  ...
  it('opens non-settings routes with the query string preserved', () => {
    handleNavigateProtocolUrl(new URL('cherrystudio://navigate/app/chemsmart?x=1&y=2'))

    expect(openRouteInMainWindowMock).toHaveBeenCalledWith('/app/chemsmart?x=1&y=2')
    expect(openSettingsInMainWindowMock).not.toHaveBeenCalled()
  })
})
