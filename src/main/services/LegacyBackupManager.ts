...
class BackupManager {
  ...
  private async restoreDirect(): Promise<void> {
    ...
    try {
      ...
      // Accept the legacy product name only for historical backup compatibility.
      if (metadata.appName !== 'ChemSmart Studio' && metadata.appName !== 'Cherry Studio') {
        throw new Error('This backup file is not from ChemSmart Studio and cannot be restored')
      }
