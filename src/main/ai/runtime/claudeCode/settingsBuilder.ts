/**
 * Builds ClaudeCodeSettings from ChemSmart Studio's agent session configuration.
 *
 * Maps ChemSmart Studio's internal data model (agent sessions, providers, MCP servers,
...
const MINIMAL_CHERRY_ASSISTANT_INSTRUCTIONS =
  'You are ChemSmart Agent, the built-in helper for ChemSmart Studio. Help users understand and troubleshoot ChemSmart Studio.'
...
/**
 * Build a lightweight environment snapshot (~200 tokens) for ChemSmart Agent.
...
function buildAssistantContext(): string {
  ...
  return [
    '## Current Environment',
    `- App: ChemSmart Studio v${appVersion}`,
    `- OS: ${platform}`,
...
/**
 * Build session-level ClaudeCodeSettings from ChemSmart Studio's agent session.
 */
export async function buildClaudeCodeSessionSettings(
  ...
  // Assistant diagnostics there. Local ChemSmart Agent sessions keep the full MCP.
...
async function buildEnvironment(provider: Provider, agent: AgentEntity): Promise<Record<string, string | undefined>> {
  ...
  const env: Record<string, string | undefined> = {
    ...
    CHEMSMART_STUDIO_BUN_PATH: bunPath,
    ...
    return {
      hookSpecificOutput: {
        ...
        permissionDecisionReason:
          'Headless channel or scheduled turns cannot mutate agent configuration. Ask the user to make this change in ChemSmart Studio.'
      }
    }
  }
...
export async function buildSystemPrompt(
  ...
  if (builtinRole && !instructions) {
    ...
    if (definition?.instructions) {
      ...
      logger.error('Builtin ChemSmart Agent definition missing; using minimal fallback instructions')
...
export function buildMcpServers(
  ...
  // 5. Assistant — navigate + diagnose tools (local ChemSmart Agent sessions only)
  if (assistantMcpEnabled) {
    ...
    logger.debug('ChemSmart Agent: injected assistant MCP server', {
