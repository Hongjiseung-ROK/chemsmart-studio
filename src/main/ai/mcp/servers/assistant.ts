...
const ALLOWED_ROUTES = ['/settings', '/app/chemsmart']
...
const NAVIGATE_TOOL: Tool = {
  name: 'navigate',
  description:
    'Navigate ChemSmart Studio to a specific page. Refer to the route table in your skills for available paths.',
  inputSchema: {
    type: 'object',
    properties: {
      path: {
        type: 'string',
        description: 'The route path to navigate to, e.g. /app/chemsmart or /settings/provider'
      },
      query: {
        type: 'object',
...
const DIAGNOSE_TOOL: Tool = {
  name: 'diagnose',
  description:
    'Read ChemSmart Studio runtime state for troubleshooting. Use this to inspect app info, provider config, connectivity, logs, and MCP server status.',
  inputSchema: {
    type: 'object',
    properties: {
      action: {
