...
type InstallExtension = (extensionReference: unknown) => Promise<string>

interface ReactDevtoolsInstaller {
  installExtension: InstallExtension
  reactDeveloperTools: unknown
}

function asModuleRecord(value: unknown): Record<string, unknown> | undefined {
  return typeof value === 'object' && value !== null ? (value as Record<string, unknown>) : undefined
}

export function resolveReactDevtoolsInstaller(moduleValue: unknown): ReactDevtoolsInstaller {
  const moduleRecord = asModuleRecord(moduleValue)
  const nestedDefault = asModuleRecord(moduleRecord?.default)
  const installExtensionCandidate =
    typeof moduleRecord?.default === 'function' ? moduleRecord.default : nestedDefault?.default
  const reactDeveloperTools = moduleRecord?.REACT_DEVELOPER_TOOLS ?? nestedDefault?.REACT_DEVELOPER_TOOLS

  if (typeof installExtensionCandidate !== 'function' || reactDeveloperTools === undefined) {
    throw new TypeError('electron-devtools-installer exposed an unsupported module shape')
  }

  return {
    installExtension: installExtensionCandidate as InstallExtension,
    reactDeveloperTools
  }
}
...
async function installReactDevtools() {
  try {
    ...
    const { installExtension, reactDeveloperTools } = resolveReactDevtoolsInstaller(installerModule)
    const name = await installExtension(reactDeveloperTools)
