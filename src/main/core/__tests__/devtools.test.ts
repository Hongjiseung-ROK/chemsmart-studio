...
import { installDevtoolsExtensions, resolveReactDevtoolsInstaller } from '../devtools'
...
describe('resolveReactDevtoolsInstaller', () => {
  it('accepts the ESM-shaped installer module', () => {
    const installExtension = vi.fn()
    const reactDeveloperTools = { id: 'react-devtools' }

    expect(
      resolveReactDevtoolsInstaller({
        default: installExtension,
        REACT_DEVELOPER_TOOLS: reactDeveloperTools
      })
    ).toEqual({
      installExtension,
      reactDeveloperTools
  ...
  it('accepts the nested CommonJS-shaped installer module', () => {
    const installExtension = vi.fn()
    const reactDeveloperTools = { id: 'react-devtools' }
  ...
  it('rejects unsupported installer module shapes', () => {
    expect(() => resolveReactDevtoolsInstaller({ default: {} })).toThrow(
      'electron-devtools-installer exposed an unsupported module shape'
    )
  })
})
