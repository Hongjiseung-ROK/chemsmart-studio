...
/**
 * Require this process to be the primary ChemSmart Studio instance.
 *
 * Claims Electron's single-instance lock via `app.requestSingleInstanceLock()`.
 * If another ChemSmart Studio process already holds the lock, this function
...
export function requireSingleInstance(): void {
  ...
  logger.info('Another ChemSmart Studio instance already holds the single-instance lock; exiting')
  application.quit()
  process.exit(0)
}
