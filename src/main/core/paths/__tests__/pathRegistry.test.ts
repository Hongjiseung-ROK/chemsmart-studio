...
import { buildShortSocketRuntimePath, shouldAutoEnsure } from '../pathRegistry'

describe('pathRegistry short local socket runtime', () => {
  it('keeps the longest ChemSmart helper socket below the macOS Unix-domain limit', () => {
    const runtime = buildShortSocketRuntimePath(
      '/var/folders/ab/example-user-hash/T',
      '/Users/example/Library/Application Support/ChemSmartStudioDevGoalControlled'
    )
    const socketPath = `${runtime}/controlled-sidecar-test-0123456789abcdef.sock`

    expect(socketPath.length).toBeLessThan(104)
    expect(runtime).toMatch(/^\/tmp\/chemsmart-studio-[0-9a-f]{12}$/)
  })

  it('isolates concurrent development profiles without exposing their names', () => {
    const first = buildShortSocketRuntimePath('/var/folders/example/T', '/profiles/first')
    const second = buildShortSocketRuntimePath('/var/folders/example/T', '/profiles/second')
...
describe('pathRegistry.shouldAutoEnsure', () => {
  describe('cherry-owned directories — should auto-ensure', () => {
    ...
    it('returns true for the ChemSmart Studio agent workspace', () => {
      // The agent sidecar runs with this as its working directory, and chemsmart writes its
      // workspace-scoped method projects and CHEMSMART.md inside it, so Studio must create it.
      expect(shouldAutoEnsure('feature.chemsmart_studio.workspace')).toBe(true)
    })

    it('returns true for the ChemSmart Studio active-project marker', () => {
      expect(shouldAutoEnsure('feature.chemsmart_studio.active_project_file')).toBe(true)
    })
