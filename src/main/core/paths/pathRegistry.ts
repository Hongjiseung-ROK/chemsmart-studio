...
export function buildShortSocketRuntimePath(sysTemp: string, appUserData: string): string {
  const profileDigest = createHash('sha256').update(appUserData).digest('hex').slice(0, 12)
  return path.join(path.parse(sysTemp).root, 'tmp', `chemsmart-studio-${profileDigest}`)
}
...
export function buildPathRegistry() {
  ...
  const chemsmartStudioRuntime = isMac
    ? buildShortSocketRuntimePath(sysTemp, appUserData)
    : path.join(appTemp, 'chemsmart-studio')
  ...
  return Object.freeze({
    ...
    // ChemSmart Studio molecule workspace and AgentSession sidecar
    'feature.chemsmart_studio.projects': path.join(appUserDataData, 'ChemSmartStudio', 'Projects'),
    'feature.chemsmart_studio.active_project_file': path.join(
      appUserDataData,
      'ChemSmartStudio',
      'active-project.json'
    ),
    // The agent sidecar's working directory. chemsmart resolves its own workspace-scoped method projects
    // (`.chemsmart/{gaussian,orca,xtb}/*.yaml`) and its CHEMSMART.md rules file relative to this directory,
    // which is how Studio keeps them out of the researcher's own ~/.chemsmart. Distinct from
    // `feature.chemsmart_studio.projects`, which holds .cmsproj molecule bundles.
    ...
    'feature.chemsmart_studio.bridge.python_file': path.join(
      appExtraResources,
      'chemsmart-studio',
      'bridge',
      '.venv',
