...
describe('WINDOW_TYPE_REGISTRY Main window — frame contract', () => {
  ...
  it('permits the compact molecular workbench at 760×560', () => {
    const result = mergeWindowOptions(WindowType.Main)
    expect(result.minWidth).toBeLessThanOrEqual(760)
    expect(result.minHeight).toBeLessThanOrEqual(560)
  })
})

describe('WINDOW_TYPE_REGISTRY retired consumer windows', () => {
  it('does not register quick assistant or selection windows', () => {
    expect(WINDOW_TYPE_REGISTRY[WindowType.QuickAssistant]).toBeUndefined()
    expect(WINDOW_TYPE_REGISTRY[WindowType.SelectionAction]).toBeUndefined()
  })
})
