...
import { ChemSmartAgentSeeder } from '@data/db/seeding/seeders/chemSmartAgentSeeder'
...
describe('ChemSmartAgentSeeder', () => {
  const dbh = setupTestDatabase()

  it('uses a constant version so preset changes cannot bypass deletion memory', () => {
    expect(new ChemSmartAgentSeeder().version).toBe('1')
  })

  it('registers the default model before the ChemSmart Agent in the production registry', () => {
    const modelSeederIndex = seeders.findIndex((seeder) => seeder instanceof CherryAiDefaultModelSeeder)
    const assistantSeederIndex = seeders.findIndex((seeder) => seeder instanceof ChemSmartAgentSeeder)
  ...
  it('creates the builtin agent with a default system session and workspace in a fresh library', () => {
    new ChemSmartAgentSeeder().run(dbh.db)
  ...
  it('skips when any active agent exists and SeedRunner still journals the one-time eligibility check', () => {
    ...
    new SeedRunner(dbh.db).runAll([new ChemSmartAgentSeeder()])
    ...
    expect(journal?.value).toMatchObject({ version: new ChemSmartAgentSeeder().version })
  })
  ...
  it('seeds after an unrelated seeder closes bootstrap with no prior library history', () => {
    ...
    runner.runAll([new ChemSmartAgentSeeder()])
  ...
  it('references the CherryAI default model when seeded after CherryAiDefaultModelSeeder', () => {
    new SeedRunner(dbh.db).runAll([new CherryAiDefaultModelSeeder(), new ChemSmartAgentSeeder()])
