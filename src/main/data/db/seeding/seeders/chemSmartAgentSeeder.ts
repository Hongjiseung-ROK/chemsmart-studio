...
export class ChemSmartAgentSeeder implements ISeeder {
  // Stable journal key retained so existing profiles do not seed a duplicate
  // built-in Agent after the product-facing rebrand.
  readonly name = 'cherryAssistant'
  readonly description = 'Insert the builtin ChemSmart Agent agent for profiles with no prior agent-library history'
  ...
  run(db: DbType): void {
    db.transaction((tx) => {
      ...
      const row = agentService.createAgentTx(tx, agentId, {
        id: agentId,
        type: 'claude-code',
        name: CHEMSMART_AGENT_SEED.name,
        description: '',
        instructions: '',
        model: this.getCherryAiDefaultModelId(tx),
        configuration: { ...CHEMSMART_AGENT_SEED.configuration }
      })

      if (!row) {
        throw new Error('insert succeeded but select returned no builtin ChemSmart Agent row')
      }
