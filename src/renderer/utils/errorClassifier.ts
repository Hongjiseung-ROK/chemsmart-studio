...
export function classifyError(error?: SerializedError, providerId?: string): ErrorClassification {
  ...
  if (
    ...
    return { category: 'network', i18nKey: 'error.diagnosis.network', navTarget: '/settings/provider' }
  }
  ...
  if (
    ...
    return { category: 'proxy', i18nKey: 'error.diagnosis.proxy', navTarget: '/settings/provider' }
  }
  ...
  if (msg.includes('mcp server') || msg.includes('mcp connection') || msg.includes('mcp error')) {
    return { category: 'mcp', i18nKey: 'error.diagnosis.mcp', navTarget: '/settings/provider' }
  }
