...
- **Main-only** concerns stay in `MainWindowRuntime`, explicitly outside `useWindowRuntime`: the boot spinner + `init` timer teardown (paired with markup only `main/index.html` creates), `useStorageMonitorNotification`, and `useTopicNamingErrorNotification` (main-window-targeted toasts that must not duplicate across windows).
...
|---|---|---|
| `main` | `MainApp` | `StudioWorkbench` |
| `subWindow` | `SubWindowApp` | inactive Studio retirement view after pooled init data arrives |
