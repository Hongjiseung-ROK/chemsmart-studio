# ChemSmart Studio Design System
...
ChemSmart Studio is a computational-chemistry IDE built on the shared `@cherrystudio/ui` component and token foundation. Its product language combines a Cursor-like IDE information architecture with molecule-building workflows familiar to GaussView users. The interface recedes so the molecular geometry, scientific state, and researcher decisions remain central. Clean surfaces, subtle borders, dense but legible controls, and restrained color create a trustworthy research tool in both light and dark modes.

The typography system is single-track: `var(--font-family-body)` and `var(--font-family-heading)` currently resolve to the same primary UI font token. Console, job, identifier, and coordinate components own their mono font stack locally. This gives the product one calm UI voice while preserving precision where scientific data requires it.

The Studio chrome is deliberately neutral. Primary actions use `var(--color-primary)` as the strongest action color, while shared buttons may define neutral strong fills for their own hierarchy. Chromatic departures are semantic: amber means a decision is waiting, red means an actual failure, green means a verified success, and blue means neutral information. Every colored state must also carry a label or icon; color never bears meaning alone.
...
### Studio IDE Product Contract

`/app/chemsmart` is the only product workspace. The visible shell consists of:

- an integrated title bar with ChemSmart Studio, project name, committed revision, decision count, run state, and panel toggles;
- a compact `Settings / View / Server / Agent / Info` toolkit line directly below the title bar;
...
The layout contract is expressed by `StudioLayoutTier = wide | focused | viewport-only`, `StudioPaneId = explorer | properties | agent | decisions | console | jobs | problems`, `StudioPaneIntent`, and `StudioPanePresentation`. The Stage is always the spatial priority. Explorer and ChemSmart Agent are open by default in a wide window; the bottom panel opens only through explicit researcher action. Background agent or job events may update badges and content but must never steal keyboard focus or open a pane.

Pane intent is independent of window size. Resizing may change only how an open pane is presented:

- docked Explorer, Agent, and bottom-panel sizes are stored as normalized workspace ratios, initially 18%, 28%, and 30%;
- compact Agent and Console presentations are workspace-relative Sheets at 44% width and 42% height;
- entering or leaving a compact layout never clears another pane's open intent or splitter ratio;
- only one compact Sheet is rendered at a time, but switching Sheets preserves the other dock intents;
- Agent and Console toggles stay directly available in the title bar at every layout size;
- Escape closes the current Sheet without moving Stage or input focus, while resize itself never closes a pane or changes focus.
...
- `Committed rN` means the persisted molecule revision is displayed.
- Run and replay labels include their run/frame identity and must never be visually presented as committed state.
- Researcher and Agent edits are shown as a recoverable draft layered over the committed revision. Each `StudioDraftEntry` includes a main-generated, path-free `StudioPreviewSummary`; the Save and Run review dialogs present the ordered history and its Researcher/Agent source.
- `Draft · N changes · committed rN` means Three.js is displaying the main-owned draft while disk persistence still points to the labeled committed revision. `Apply` produces one durable revision; editing gestures never do.
- Opaque IDs may support audit details, but they never replace the human summary of element, coordinate, bond, and constraint changes.

The Builder follows direct-manipulation molecular-editor conventions:

- the element control opens a complete 118-element periodic table with search and keyboard navigation;
- choosing an element immediately arms insertion; there is no separate “Insert atom” button;
- clicking empty space places an independent atom, while clicking an existing atom creates an explicit bond using the selected order and coordination guide;
- element replacement is a separate, explicit tool;
- coordination geometry supplies placement direction only and never fabricates implicit hydrogen;
- fragment and ring templates contain explicit atom and bond topology, including explicit hydrogen.

All pointer targets are at least 32×32px. Interactive controls have visible focus rings, semantic labels, and complete keyboard navigation. Panel toggle labels, `aria-expanded`, and rendered state must agree. `Cmd/Ctrl+B` toggles Explorer, `Cmd/Ctrl+J` toggles the bottom panel, `Cmd/Ctrl+Shift+P` opens the Studio command palette, and Escape closes the active transient panel or Sheet.
...
### Studio Workbench Patterns

These patterns define the current product workspace and should be treated as design-system requirements.

**Molecule Stage**
- Keep the Stage flat, dark-neutral, and visually dominant. Floating viewport tools use `bg-popover`, `border-border`, and `shadow-md`; they do not reserve a large permanent band around the canvas.
- The editor tab and geometry-state label remain visible whenever the Stage is visible.
...
- Amber is reserved for an unresolved decision. Red is reserved for a denied operation only when it is also a failure, or for an actual validation/persistence/execution failure.
- Opening Decisions is an explicit user action; new background decisions update counts without stealing focus.
...
ChemSmart Studio uses a dual depth system: **surface color layering** for structural hierarchy and **box-shadows** for interactive feedback (hover states, floating elements).
...
- Responsive reduction removes secondary chrome before it removes scientific context: the active molecule tab, geometry-state label, Stage, and current tool remain legible.
...
- "Create a dark-mode Jobs panel on `var(--color-background)`. Run/frame identifiers use the product mono stack, pending final geometry uses warning text plus an explicit label, failures use the error surface tokens, and replay controls remain keyboard reachable."
