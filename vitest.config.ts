...
export default defineConfig({
  test: {
    projects: [
      ...
      // packages/chem-molecular-engine — the pure scene layer only; MoleculeCanvas needs a GPU.
      {
        extends: true,
        resolve: {
          alias: {
        ...
        test: {
          ...
          environment: 'node',
          include: [
            'packages/chem-molecular-engine/**/*.{test,spec}.{ts,tsx}',
            'packages/chem-molecular-engine/**/__tests__/**/*.{test,spec}.{ts,tsx}'
          ]
        }
      },
      // packages/ui 单元测试配置
