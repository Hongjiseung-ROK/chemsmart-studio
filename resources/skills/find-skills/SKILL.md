...
If `npx` is **not found**, fall back to the bundled bun shipped with ChemSmart Studio.
ChemSmart Studio sets the `CHEMSMART_STUDIO_BUN_PATH` environment variable pointing to its
bundled bun binary. Use it as follows:

```bash
if [ -n "$CHEMSMART_STUDIO_BUN_PATH" ] && [ -x "$CHEMSMART_STUDIO_BUN_PATH" ]; then
  "$CHEMSMART_STUDIO_BUN_PATH" x skills <subcommand> [args]
else
  echo "Error: Neither npx nor bundled bun found. Install Node.js or use ChemSmart Studio's bundled toolchain."
fi
```

For example, `npx skills find react` becomes `"$CHEMSMART_STUDIO_BUN_PATH" x skills find react`.
